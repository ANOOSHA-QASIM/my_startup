...
>((props: any, ref) => {
  ...
  const { active, payload, hideLabel = false, hideIndicator = false, indicator = "dot", label, labelKey, nameKey } = props
  ...
  return (
    <div ref={ref} className="grid min-w-[8rem] items-start gap-1.5 rounded-lg border bg-background px-2.5 py-1.5 text-xs shadow-xl">
      ...
      {(payload as any[]).map((item: any, index: number) => {
        ...
        return (
          <div key={item.dataKey} className="flex w-full flex-wrap items-stretch gap-2">
            {!hideIndicator && <div className="h-2.5 w-2.5 rounded-[2px]" style={{ backgroundColor: indicatorColor }} />}
...
>((props, ref) => {
  const { config } = useChart()
  const { payload, hideIcon = false, verticalAlign = "bottom", nameKey, className } = props
  if (!payload?.length) return null

  return (
    <div ref={ref} className={`flex items-center justify-center gap-4 ${verticalAlign === "top" ? "pb-3" : "pt-3"} ${className}`}>
      {payload.map((item: RechartsPrimitive.LegendPayload) => {
        const key = `${nameKey || item.dataKey || "value"}`
        const itemConfig = getPayloadConfigFromPayload(config, item, key)
        return (
          <div key={item.value} className="flex items-center gap-1.5">
            {!hideIcon && itemConfig?.icon ? <itemConfig.icon /> : <div className="h-2 w-2 rounded-[2px]" style={{ backgroundColor: item.color }} />}
            {itemConfig?.label}
          </div>
        )
      })}
